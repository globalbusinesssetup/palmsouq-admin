<template>
  <div class="custom-dropdown" :class="{ open: dropdownOpen }">
    <span
      ...
      :style="rounded ? { borderRadius: '999px' } : undefined"
    >
      {{ selectedValue }}
      <i
    ...
    <div v-if="dropdownOpen" class="dropdown-wrapper">
      ...
      <div
        ...
        :id="currentId"
      >
        <input v-if="searching" ref="searcBox" type="text" v-model="searched" />
        <ul>
          <li
...
export default {
  ...
  methods: {
    ...
    clicked(index, obj) {
      ...
      if (
        (index !== "0" && this.currentKey !== index) ||
        (this.defaultNull && this.currentKey !== index)
      ) {
