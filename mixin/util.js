...
export default {
  ...
  methods: {
    ...
    phpDecryption(encrypted) {
      ...
      var decrypted = CryptoJS.AES.decrypt(encrypted, DataKey, {
        iv: DataVector,
      });
    ...
    priceFormatting(price) {
      if (parseInt(this.currencyPosition) === this.currencyPositionsIn.PRE) {
        return (
          this.currencyIcon +
          this.decimalSeparator(price, this.setting?.decimal_format)
        );
      }
      return (
        this.decimalSeparator(price, this.setting?.decimal_format) +
    ...
    decimalSeparator(price, decimalSeparator = "en-US") {
      ...
      return parseFloat(price).toLocaleString(decimalSeparator, {
    ...
    getThumbImageURL(image) {
      ...
      return (
        this.$store.state.imgSrcUrl + this.$store.state.thumbPrefix + image
      );
    },
    ...
    getSubCategory(index) {
      return (
        this.$store.getters["sub_category/ALL_SUB_CATEGORIES"][index].title ||
        ""
      );
    },
    getBrand(index) {
