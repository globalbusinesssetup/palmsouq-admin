<template>
  <setting-layout active-route="miscellaneous" class="mb-5">
    <template v-slot:rightArea>
      <div
        v-if="$store.state.admin.activated"
        class="f-wrap mb-20 mb-sm-15 info-msg dply-felx gap-10"
      >
        <p class="">
      ...
      >
        ...
        <div class="input-wrapper single-line">
          ...
          />
          <span class="error" v-if="!!!result.code && hasError">
...
export default {
  ...
  computed: {
    isDemo() {
      console.log(process?.env?.isDemo);
  ...
  methods: {
    ...
    generateStateList() {
      this.stateList = this.countryList["AE"]?.states;
    },
    selectedCountry(evt) {
      console.log(evt, this.cbSetting.default_country);
