<template>
  <div v-if="sidebarPermissions" class="sidebar">
    <div v-if="childrenOpened" class="child-layer" @click="closeChild" />
    <ul class="sb sb-2">
      <li
        v-for="(value, index) in sidebarsArr"
        :key="index"
        :class="[
          { 'child-open': value.open },
          { 'child-active': value.childActive },
          { active: isCurrentRoute(value) },
        ]"
      >
        <nuxt-link
        ...
        >
          ...
          <span class="dply-felx grow">
            <span class="dply-felx grow">
              {{ value.title }}
              <span v-if="checkBadge(value)" class="plugin-badge">{{
                $t("ship.pls")
              }}</span>
            </span>

            <i v-if="value.children" class="icon arrow-down" />
          </span>
        </nuxt-link>

        <ul v-if="value.children && value.open" class="child">
          <li
    ...
    <transition name="fade" mode="out-in">
      <div v-if="sidebarOpen" class="layer" @click.prevent="hideSidebar" />
    </transition>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      sidebarsArr: [
        ...
        {
          path: "pos",
          title: this.$t("ship.pos"),
          icon: "pos",
          open: false,
          gate: "pos",
          badge: !!process?.env?.isDemo,
          children: [
            {
              path: "pos/manager",
              title: this.$t("ship.pm"),
              icon: "pos-manager",
              gate: "pos",
            },
            {
              path: "pos/order",
              title: this.$t("ship.po"),
              icon: "orders",
              gate: "pos",
            },
            {
              path: "pos/configuration",
              title: this.$t("ship.pc"),
              icon: "setting",
              gate: "pos_setting",
            },
          ],
  ...
  methods: {
    ...
    reRenderActiveChild() {
      ...
      if (sidbearI > -1) {
        this.sidebarsArr.splice(sidbearI, 1, {
          ...this.sidebarsArr[sidbearI],
          ...{ childActive: false },
        });
      }
